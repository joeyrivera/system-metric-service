...
resource "aws_key_pair" "resource_metric_service" {
  key_name   = "resource_metric_service_key"
  public_key = ""
...
data "archive_file" "test_lambda_code" {
  ...
  output_path = "${path.module}/.terraform/archive_files/lambda_function_payload.zip"
}
...
resource "aws_lambda_function" "get_shard_health" {
  ...
  source_code_hash = filebase64sha256(data.archive_file.test_lambda_code.output_path)
